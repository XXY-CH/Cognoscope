...
核心叙事是：研究者负责搜索后导入、阅读和判断；环境负责整理、关联、核验提示和回读路径。图谱是导航线索，证据矩阵是可引用材料的权威入口。
...
| 5 | `READ-04` 批注高亮层与真实 TOC | `done` | locator 在 PDF/EPUB 原文上可见、可回读 | `C2`, `R1` |
...
`KG-05`、`AI-03`、`KG-06` 和 `MON-02` 已完成当前可执行的实现与静态验证：图谱证据锚点、文件级 QA、来源失效传播和 monitor 会话合并均已落地。`READ-04` 已完成真实 PDF/EPUB 与多视口 reader 验收并通过本轮 Integration/Release checkpoint；`KG-06`、`MON-02` 和完整 `UX-06` 仍保留各自的浏览器、摄像头或跨页面闭环边界，下一轮从 `AG-STATE` 恢复并按依赖推进 `UX-06` 和 `P4`。
