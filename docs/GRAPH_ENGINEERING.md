...
浏览器门保持 `blocked`，不能用静态检查替代集成验收。本轮真实浏览器门、KG-06
来源生命周期和 P4 矩阵回读门已通过，只有 MON-02 摄像头运行时仍作为独立边界保留。
...
- `C5` 多视图个人研究地图：资料、论证、比较、演化和局部探索共享一份关系投影，
  图谱关系仍是导航线索，证据矩阵负责唯一核验。
- `RM-00` Phase 006 多视图研究地图设计契约：资料、论证、比较、演化和局部探索
  共用一份关系投影，并把证据矩阵保留为唯一核验入口。
- `RM-01` 关系语义与投影适配器：旧边按端点语义投影为 `relates`、`mentions`、
  `contains` 或显式 `unknown`；来源、状态和 evidence/condition 引用只保留稳定 ID，
  证据来源失效与 locator 不可回读时降级，图谱关系始终不能进入 citation-ready 输出。
...
flowchart TD
  ...
  RM1 --> RM2["RM-02 论证视图\ndone"]
  RM1 --> RM3["RM-03 资料分层视图\ndone"]
  RM1 --> RM4["RM-04 比较/演化视图\ndone"]
  RM4 --> KG6["KG-06 来源生命周期\ndone"]
  KG6 --> P4["P4 证据矩阵闭环与真实材料验收\ndone"]
  P4 --> P5["P5 真正的科研交付能力\ndeferred"]
```
...
| 2 | `RM-01` 关系语义与投影适配器 | `done` | 旧边按端点语义投影，稳定引用和证据状态不越过矩阵核验门 | `RM-00`, `C4`, `C5` |
...
| 4 | `KG-06` 失效来源与状态传播 | `done` | 删除/修改来源后，图谱、矩阵和研究状态统一降级；真实软删除、恢复和硬清理已回归 | `KG-05`, `D3` |
| 7 | `P4` 证据矩阵闭环与真实材料验收 | `done` | 来源检查器、核验复制门和矩阵 → Reader 回读在真实 PDF 下闭环 | `KG-05`, `KG-06`, `UX-06` |
| — | `P5` 科研成果交付能力 | `deferred` | 外部检索、TeX/期刊检查、引用扫描和写作交付适配；必须先通过 O2 研究质量门 | `P4`, `O2` |
...
| 8 | `UX-06` 真实文档视觉验收 | `done` | 390/768/1280、键盘、主题、离线、减弱动效和图谱/矩阵回读均通过 | `KG-05`, `C4` |
| 9 | `EXT-01` 外部检索与下载 | `deferred` | 后续阶段接入，不影响本地闭环 | `O4` 稳定后再立项 |
| 10 | `TEX-01` TeX / 期刊格式和引用扫描 | `deferred` | 后续科研交付阶段接入，不在当前阅读闭环中偷渡 | `P4` |

`RM-00` 至 `RM-06`、`KG-06` 和 `P4` 已完成运行时实现与集成验收，`UX-06` 的真实 PDF、多视口、键盘、主题、
减弱动效、五视图切换、来源检查器和图谱 → 证据矩阵 → Reader 回读均已通过；不得把相似度或矩阵共现自动升级为证据。
`O2` 仍保留跨论文反例、方法和数据集独立性质量门；`MON-02` 的 Python 摄像头运行时仍独立保持边界，`P5` 在这些质量门
满足后再进入科研交付规划。
