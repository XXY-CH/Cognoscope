...
| 3 | `RM-02` 主张中心论证视图 | `next` | 主张、关系和局部证据锚点可解释并回到 Reader | `RM-01`, `KG-05` |
| 4 | `RM-03` 资料分层视图 | `next` | 研究范围、主题簇、论文和概念分层呈现 | `RM-01` |
| 5 | `RM-04` 比较摘要与判断演化视图 | `next` | 比较矩阵摘要和研究判断变化保持可回读 | `RM-01`, `T4` |
...
`RM-00` 已完成文档设计，`RM-01` 已完成运行时关系适配和静态验证。后续只能在其统一投影合同上实现 `RM-02`、`RM-03`、`RM-04`，不得把相似度或矩阵共现自动升级为证据。浏览器门仍独立保持 `blocked`，不能用静态检查替代真实图谱/矩阵/Reader 回读验收。
