...
| **Phase 003** | **知识图谱分级与 UI 设计**（研究空间 / 文献 / 概念 / 证据 / 综合判断） | 🛠 首轮实现进行中（筛选、关系来源、节点检查器已落地） |
| **Phase 004** | **系统级 UI 重设计**（页面分级 / 视觉表面 / 动画语言 / 全路由响应式） | 🛠 首轮实现进行中（全局壳层、动效 token、移动布局已落地） |
| **Phase 005** | **AI 时代科研环境叙事**（当前研究状态首页 / 资料库支撑路由） | 🛠 第二条垂直切片已接通（v10 研究信号/线索、会话后整理门槛、回读摘要、移动验收） |
| — | **可自定义快捷键** + 全局/阅读热键 | ✅ 完成 |
...
- [x] IndexedDB **v9**：`files` / `fileBlobs` / `sessions` / `annotations` / `bookmarks` / `evidenceMatrices` / `evidenceRows` / `evidenceAnalyses`
...
- [x] 矩阵打开、提取、分析的过期请求保护与串行 IndexedDB 写入；探针覆盖 v7→v9 增量 schema 契约
...
7. AI 配置已落盘；阅读器内 QA/整理习得仍待接，证据矩阵与研究分析使用独立的结构化提议请求。
