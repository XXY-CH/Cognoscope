...
**主路径现状**：导入 PDF/EPUB → 打开阅读 → 翻页/缩放/划词批注与书签 → 文内搜索与全屏 → 已读行数与会话写入 IndexedDB → 仪表盘读真实会话；打开阅读时可请求 `monitor` 启停检测（服务未起则静默失败）。进入知识图谱后可搜索论文/关键词、查看关系依据并回到原文；在文件目录或图谱论文簇选择三至五篇论文可进入证据矩阵，生成候选结论、核对原文、回读、复制引用材料，再基于已确认行整理研究结论、局限/矛盾与研究空白/机会。阅读器内 QA/整理习得、批注原文高亮层、Toc 真实目录、monitor 会话合并进 IndexedDB 仍待接。
