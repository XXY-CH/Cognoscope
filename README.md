...
Congnoscope 是基于浏览器的**学术研究工作台**：本地优先管理 PDF / EPUB，以阅读和批注捕获证据，再通过证据矩阵与多视图研究地图整理可回读的研究判断。仓库中的 `xuesen` 脚本名和 `cognoscope` Python 包名是运行时兼容标识，不是产品展示名。

| 文档 | 用途 |
|---|---|
| [`UI_spec.md`](./UI_spec.md) | 产品与视觉规范（权威；部分产品迭代见下） |
| [`PROGRESS.md`](./PROGRESS.md) | 进度、待办、技术债、变更记录 |
| [`docs/DOCS_INDEX.md`](./docs/DOCS_INDEX.md) | 当前文档入口、权威关系与归档说明 |
| [`docs/archive/HANDOFF.md`](./docs/archive/HANDOFF.md) | 历史交接记录（只读归档） |
| [`docs/archive/AI_FEATURES_CHECKLIST.md`](./docs/archive/AI_FEATURES_CHECKLIST.md) | 历史 AI 链路清单（只读归档） |
...
| 存储 | IndexedDB（`idb`）v11：文件 / blob / 会话 / 批注 / 书签 / 证据矩阵 / 研究产物 / 文件级 QA |
...
- **研究地图**（`/knowledge-graph`）：资料、论证、比较、演化和局部探索五种任务视图；支持按论文标题/主题搜索、关系来源/理由查看、局部力导向探索，以及从主张/节点详情回到原文或证据矩阵
...
- **研究地图 Phase 003/006**：将图谱事实拆为“研究空间 → 文献来源 → 语义概念 → 证据锚点 → 综合判断”五级数据层，并在资料、论证、比较、演化和探索视图中投影；关系继续只作导航线索，证据矩阵仍是可引用材料的权威入口。
...
- **AI 时代科研环境 Phase 005（会话后整理）**：IndexedDB v10/v11 新增会话整理、研究信号、待审阅线索和文件级 QA；离开阅读后按原文摘录/定位门槛保存用户批注，整理结果有结构化栏目与 Markdown 导出，首页可回读来源并查看整理正文。

当前仍受运行环境限制的事项：完整图谱/矩阵浏览器回归、monitor 摄像头运行时和图谱关系的批注级证据深化（详见 `PROGRESS.md`）。问答与整理结果已支持 Enter 发送、Markdown 和常见 LaTeX/MathML；来源证据仍以矩阵核验为准。
